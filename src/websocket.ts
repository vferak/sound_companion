...
export const websocket = new Elysia().ws("/ws", {
  ...
  open(ws) {
    ...
    console.log(
      `IP: ${ws.data.request?.headers?.get("x-forwarded-for") || ws.data.request?.headers?.get("host") || "unknown"}`,
    );
    console.log("========================================\n");
  ...
  message(ws, { message, type }) {
    ...
    if (type === "broadcast") {
      ...
      console.log(
        `📡 Broadcasting to ${recipientCount} other client(s) in channel: ${channel}`,
      );
  ...
  close(ws) {
    ...
    const sessionDuration = ws.data.connectedAt
      ? Math.round(
          (Date.now() - new Date(ws.data.connectedAt).getTime()) / 1000,
        )
      : 0;
  ...
  wsClients.forEach((client: any) => {
    if (client.readyState === 1) {
      ...
      try {
        ...
        console.error(
          `Failed to send message to client ${client.data.clientId}:`,
          error,
        );
        failedCount++;
      }
...
export function broadcastToChannel(
  ...
  if (failedCount > 0) {
    console.log(
      `⚠️ Failed to send to ${failedCount} client(s) in channel ${channel}`,
    );
  }
  if (sentCount > 0) {
    console.log(
      `✅ Successfully sent to ${sentCount} client(s) in channel ${channel}`,
    );
  }
  return { sent: sentCount, failed: failedCount };
}
...
export function findClientById(clientId: string): any {
  return Array.from(wsClients).find(
    (client: any) => client.data.clientId === clientId,
  );
}
...
export function getChannelClientCount(channel: string): number {
  return Array.from(wsClients).filter(
    (client: any) => client.data.channel === channel,
  ).length;
}
